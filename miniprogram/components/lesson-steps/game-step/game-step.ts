...
Component({
  ...
  data: {
    ...
    gameResult: null as { score: number; correct: number; wrong: number } | null,
    playing: false,
    completed: false,
    /** 当前轮的 3 张卡片 */
    moles: [] as MoleCard[],
  ...
  lifetimes: {
    attached() {
      const gameType = (this.properties.step?.content?.gameType as string) || 'whack-a-mole';
  ...
  methods: {
    ...
    nextRound() {
      ...
      const moles: MoleCard[] = shuffled.map((word, i) => ({
        id: i,
        word,
    ...
    /** 点击卡片 */
    onHitMole(e: WechatMiniprogram.BaseEvent) {
      const { id } = e.currentTarget.dataset;
      const mole = this.data.moles.find(m => m.id === id);
      if (!mole || mole.hit) return;

      const updated = this.data.moles.map(m =>
        m.id === id ? { ...m, hit: true } : m
      );
