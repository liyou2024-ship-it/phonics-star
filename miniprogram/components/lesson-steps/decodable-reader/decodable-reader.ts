...
import { getReaderById } from '../../../services/course';
import { DecodableReader } from '../../../types';
...
Component({
  ...
  data: {
    reader: null as DecodableReader | null,
  ...
  methods: {
    loadReader() {
      const readerId = this.properties.step?.content?.readerId as string;
      if (!readerId) {
    ...
    buildSentences(raw: string[]): SentenceItem[] {
      return raw.map((text) => ({
        original: text,
    ...
    buildFallbackSentences(raw: string[]): SentenceItem[] {
      return this.buildSentences(raw);
    },

    /** 点击单词 */
    onReadWord(e: WechatMiniprogram.BaseEvent) {
    ...
    /** 阅读整句 */
    onReadSentence(e: WechatMiniprogram.BaseEvent) {
      ...
      const clickedWordCount = sentences.reduce(
        (sum, s) => sum + s.words.filter(w => w.clicked).length,
        0
      );
    ...
    onComplete() {
      ...
      const allRead = sentences.every(s =>
        s.words.every(w => w.clicked)
      );
