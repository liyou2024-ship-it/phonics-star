...
import { getMockEvaluator } from '../../../modules/speech/mock-evaluator';
import { SpeechEvaluationResult } from '../../../types';

let recorderManager: WechatMiniprogram.RecorderManager | null = null;
let playbackCtx: WechatMiniprogram.InnerAudioContext | null = null;
let durationTimer: ReturnType<typeof setInterval> | null = null;

function getRecorder(): WechatMiniprogram.RecorderManager {
  if (!recorderManager) {
...
function getPlayback(): WechatMiniprogram.InnerAudioContext {
  if (!playbackCtx) {
...
Component({
  ...
  data: {
    ...
    tempFilePath: '',
    evaluating: false,
    evalResult: null as SpeechEvaluationResult | null,
    hasRecorded: false,
  },
  ...
  methods: {
    ...
    async onEvaluate() {
      ...
      try {
        const evaluator = getMockEvaluator();
        const targetWord = (this.properties.step?.content?.targetWord as string) || 'cat';
