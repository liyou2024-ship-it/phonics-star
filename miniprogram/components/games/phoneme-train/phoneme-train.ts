// import type { PhonemeTrainStation } from '../../../modules/games/game-data';
...
Component({
  ...
  data: {
    ...
    totalStations: 5,
    currentWord: '',
    letterTiles: [] as string[],
    trainCars: [] as TrainCar[],
    score: 0,
    correctCount: 0,
    wrongCount: 0,
    maxCombo: 0,
  ...
  methods: {
    ...
    nextStation(): void {
      ...
      const stations = this.properties.gameData as any[];
    ...
    onAddToTrain(e: WechatMiniprogram.TouchEvent): void {
    ...
    onRemoveFromTrain(e: WechatMiniprogram.TouchEvent): void {
      const { gameOver } = this.data;
      if (gameOver) return;
      const carIndex = e.currentTarget.dataset.index as number;
    ...
    onSubmit(): void {
      ...
      if (gameOver || trainCars.length === 0) return;
    ...
    onReset(): void {
      const station = (this.properties.gameData as any[])[
        this.data.currentStation - 1
      ];
      if (!station) return;
      this.setData({
        letterTiles: [...station.letters],
