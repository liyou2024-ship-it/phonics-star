...
/** 按会话类型给 duration 封顶 */
function capSessionDuration(events) {
    const groups = {};
    for (const e of events) {
        let t = 'other';
        if (e.eventType.startsWith('lesson_'))
            t = 'course';
        else if (e.eventType.startsWith('practice_'))
            t = 'practice';
        else if (e.eventType.startsWith('game_'))
            t = 'game';
        const key = `${e.sessionId}_${t}`;
        if (!groups[key])
            groups[key] = { type: t, total: 0 };
        groups[key].total += e.duration;
    }
    const caps = { course: 30 * 60, practice: 20 * 60, game: 15 * 60 };
    let total = 0;
...
function getQuestionType(payload) {
    return payload.questionType || payload.type || '';
}
...
Page({
    ...
    loadReport(range) {
        ...
        try {
            ...
            const allEvents = dates.flatMap(d => (0, analytics_1.getEventsByDate)(d));
            if (allEvents.length === 0) {
                this.setData({ loading: false, empty: true });
                return;
            }
            const progress = (0, progress_1.getProgress)();
            ...
            // 3. 完成课程数
            const completedCourses = Object.values(progress.lessonProgressMap).filter(lp => lp.status === 'completed').length;
            // 4. 练习局数
            const practiceSessions = dates.reduce((sum, d) => sum +
                (0, analytics_1.getEventsByDate)(d).filter(e => e.eventType === 'practice_completed').length, 0);
            // 5. 游戏完成次数
            const gameSessions = dates.reduce((sum, d) => sum + (0, analytics_1.getEventsByDate)(d).filter(e => e.eventType === 'game_completed').length, 0);
            // 6. 总答题数
            const totalAnswers = allEvents.filter(e => e.eventType === 'answer_correct' || e.eventType === 'answer_incorrect').length;
            // 7. 正确率（仅当前日期范围）
            ...
            const accuracy = rangeCorrect + rangeIncorrect > 0
                ? Math.round((rangeCorrect / (rangeCorrect + rangeIncorrect)) * 100)
                : 0;
            // 8. 跟读次数
            const pronunciationCount = allEvents.filter(e => e.eventType === 'pronunciation_evaluated').length;
            // 9. 已掌握音素
            ...
            // 15. 是否有 Mock 评测
            const hasMockEvaluations = allEvents.some(e => e.eventType === 'pronunciation_evaluated' && isMockPayload(e.payload));
            // 16. 本周课程完成列表
...
/* ========== 弱音素加权计算 ========== */
function computeWeakPhonemes(dates) {
    // 收集所有回答错误事件
    const errorEvents = dates.flatMap(d => (0, analytics_1.getEventsByDate)(d).filter(e => e.eventType === 'answer_incorrect'));
    // 收集跟读弱项（pronunciation_evaluated 中 score < 70）
    const pronunciationEvents = dates.flatMap(d => (0, analytics_1.getEventsByDate)(d).filter(e => e.eventType === 'pronunciation_evaluated' &&
        e.payload.overallScore < 70));
    const map = {};
    function ensure(id) {
        if (!map[id]) {
            map[id] = {
    ...
    const list = Object.values(map)
        .map(v => {
        const weakScore = v.listenErrors * 2 + v.blendErrors * 2 + v.segmentErrors * 2 + v.pronunciationWeak;
        const phoneme = (0, course_1.getPhonemeById)(v.phonemeId);
        return {
            ...v,
...
/* ========== 规则化学习建议 ========== */
function buildSuggestions(weakPhonemes, accuracy, streak, hasData) {
    const list = [];
    if (!hasData) {
...
/* ========== 本周课程完成列表 ========== */
function buildCompletedCourseList(progress, dates) {
    const result = [];
...
//# sourceMappingURL=parent-report.js.map