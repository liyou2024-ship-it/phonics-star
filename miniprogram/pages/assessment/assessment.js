...
Page({
    ...
    playCurrentAudio() {
        ...
        if (q && q.audioUrl) {
            audio_1.audio.play(q.audioUrl).catch(() => { });
        }
    },
    /** 手动重听当前题目音频 */
    onReplayAudio() {
    ...
    finishTest() {
        ...
        const correct = this.data.answers.filter((a) => a.correct).length;
        ...
        // 保存到本地
        const records = storage_1.storage.get('assessment_records') || [];
        records.push({ date: new Date().toISOString(), result });
        storage_1.storage.set('assessment_records', records);
        // 标记已完成评估 -> 首页「开始能力评估」提示窗口不再显示
        storage_1.storage.set('assessment_completed', true);
        event_bus_1.eventBus.emit('assessment_completed', { result });
